    ...
    for (run in 1:nruns) {
      if (ti == 0) {
        fname <- file.path(results_dir,
                           sprintf("%s-N%d-v%d-f%d-y%d-run%d",
                                 strategy, nodes, view, f_count, budget, run))
      } else {
        fname <- file.path(results_dir,
                         sprintf("%s-N%d-v%d-f%d-y%d-x%d-run%d",
                                 strategy, nodes, view, f_count, budget, t_count, run))
      }
