...
echo "Running merge experiments with strategy=${1:-bm}, budget=${2:-0.5}KB, runs=${NRUNS}"
# Faulty: 30% of N
FAULTY_PCT="${3:-26}"
FAULTY_COUNT=$(echo "$NODES * $FAULTY_PCT / 100" | bc)

STRATEGIES=("${1:-bm}") #"decay")
      ...
      for p in "${MERGES[@]}"; do
        if [ "$strat" = "array" ]; then
          ...
          cargo run -- -T $ROUNDS -n $NODES $strat \
            ...
            -n $NODES -x $t_count -p $p -s $ATTACK_START > "$OUTDIR/$outfile" &
          mark_done "$outfile"
        else
          ...
          for budget in "${BUDGETS[@]}"; do
            ...
            cargo run -- -T $ROUNDS -n $NODES $strat \
              -f $FORCE -t $FAULTY_COUNT -v $VIEW -u $UVIEW -m $SM \
              -n $NODES -y $budget -x $t_count -p $p -s $ATTACK_START > "$OUTDIR/$outfile" &
            mark_done "$outfile"
          done
        fi
      done
