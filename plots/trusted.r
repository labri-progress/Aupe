...
partview_plot <- function(df0, df1, df2, df3, df4, df5, f) {
    print("viewplot")
    print(dim(df1))
    print(colnames(df1))
    print(dim(df1))
    ...
    ggplot(df, aes(x = Time, y = comp, color = trusted)) + #, linetype = trusted)) +
        ...
        labs(title = paste("AupeMerge system resilience depending on t f=", f, "%", sep=""),
            x = "Time steps",
...
trust <- function(args, path, topic) {  
    ...
    #PLOTS
    print(partview_plot(merge0, merge1, merge2, merge3, merge4, merge5, f*100))
