...
data$Strat <- paste(data$Strat, data$rho, sep = "")

#data = data[data$comment %in% c("RAS"), ]


custom_colors <- c("BasaltRho1" = "#2CA02C", "BrahmsRho0" = "#FF7F00", "BrahmsRho1" = "#F66000",
"Aupe(t=0%)Rho0" = "#C77CFF", "Aupe(t=100%)" = "#00BFC4", "Aupe(t=1%)"="#FFFF00", 
"Aupe(t=5%)"="#FF3399", "Aupe(t=10%)"="#996600", 
"Aupe(t=20%)"="#006600", "Aupe(t=30%)Rho0"="#000000", 
...
levels=c("Aupe(t=0%)Rho0", "Aupe(t=30%)Rho0", "BasaltRho1", "Aupe(t=5%)", "Aupe(t=10%)",
    "Aupe(t=20%)", "Aupe(t=30%)", "Aupe(t=100%)", "Basalt", "BrahmsRho0", "BrahmsRho1")

data$Strat <- factor(data$Strat, levels = levels)
#data <- data[!(data$Strat %in% c("Aupe(t=1%)", "Aupe(t=5%)")), ]
kept <- c("Aupe(t=0%)Rho0", "Aupe(t=30%)Rho0", "BasaltRho1",
     "BrahmsRho0", "BrahmsRho1")
...
pdf("resilience_plotsTEST.pdf", width = width, height = height)
