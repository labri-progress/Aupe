...
round=200
mkdir analysis
# cargo run -- -T 2000 -n 10000 bm -G samples -f 10 -t 1000 -v $v -u $v -m 100 -n 10000 -y 10 > "analysis/bm-10-1000-10"
# cargo run -- -T 2000 -n 10000 kvs -G samples -f 10 -t 1000 -v $v -u $v -m 100 -n 10000  > "analysis/kvs-10-1000
...
do
    ...
    do
        ...
        if [ "$strat" == "bm" ]; then
            cargo run -- -T $round -n $N bm -G samples -f $ratio -t $faulty -v $v -u $v -m 100 -n $N -y $space > "analysis/bm-$faulty-$space"
        fi

        if [ "$strat" == "merge" ]; then
            cargo run -- -T $round -n $N bm -G samples -f $ratio -t $faulty -v $v -u $v -m 100 -n $N -y $space -x $trusty -p 10  > "analysis/mergebm-$faulty-$trusty-$space"
        fi
