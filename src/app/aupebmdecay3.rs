...
impl App for AupeDecay3 {
    ...
    fn init(&mut self, id: PeerRef, net: Net, init: &Self::Init) {
        ...
        if !self.is_byzantine{
            ...
            //self.sketch.debiais_stream_v3(self.view.clone());
            ...
            match msg {
                Msg::SelfNotif => {
                    ...
                    if !self.v_push.is_empty() && !self.v_pull.is_empty() {
                        ...
                        if self.is_trusted {
                            v_push = self.sketch.debiais_stream_v3(v_push, &mut self.rng, "push");
                            v_pull = self.sketch.debiais_stream_v3(v_pull, &mut self.rng, "pull");
                        }else {
                        ...
                        stream = self.sketch.debiais_stream_v3(stream, &mut self.rng);
