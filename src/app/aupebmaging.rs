...
use crate::util::{hash, sample, sample_nocopy};
use crate::util::{print_samples, sample_exclude};

use super::bitmatcher_aging::BM;
use crate::app::bitmatcher_aging::ffi::BitMatcherAging;

use crate::util::SEED2;
use std::sync::{OnceLock, Mutex};

static GLOBAL_OCCURENCE_AGING: OnceLock<Mutex<Vec<f64>>> = OnceLock::new();
...
pub enum Msg {
    ...
    PushRequest,
    MergeRequest(UniquePtr<BitMatcherAging>),
    MergeReply(UniquePtr<BitMatcherAging>),
}
...
pub struct Init {
    ...
    /// Sample memory size for the unbiasing strategy
...
impl App for AupeAging {
    ...
    fn init(&mut self, id: PeerRef, net: Net, init: &Self::Init) {
        ...
        GLOBAL_OCCURENCE_AGING.get_or_init(|| Mutex::new(vec![0.0f64; init.nodes]));
        ...
        if !self.is_byzantine {
            ...
            if let Some(occ) = GLOBAL_OCCURENCE_AGING.get() {
    ...
    fn metrics(&mut self, _net: Net) -> Self::Metrics {
        if self.is_byzantine {
            ...
            let (dkl, f1, bias_factor_err) = if let Some(occ) = GLOBAL_OCCURENCE_AGING.get() {
