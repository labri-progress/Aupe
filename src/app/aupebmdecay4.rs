...
// Réutilise la struct Init de aupebmdecay (mêmes paramètres CLI).
pub use super::aupebmdecay::Init;

/// Oracle global propre à cette variante (indépendant de celui d'aupebmdecay).
...
/// Echantillonne `n` pairs en excluant les noeuds byzantins [0, n_byzantine).
/// Sursamplonne puis filtre ; reboucle si nécessaire.
fn sample_peers_no_byz(net: &mut dyn Network<Msg>, n: usize, n_byzantine: usize) -> Vec<PeerRef> {
    let mut view: Vec<PeerRef> = Vec::with_capacity(n);
    // oversample to absorb the Byzantine fraction in one shot most of the time
    let oversample = n + n_byzantine + 1;
    let candidates = net.sample_peers(oversample);
    for id in candidates {
        if id >= n_byzantine && view.len() < n {
            view.push(id);
        }
    }
    // fallback: keep sampling until we have enough (handles extreme Byzantine fractions)
    while view.len() < n {
        let extra = net.sample_peers(n);
        for id in extra {
            if id >= n_byzantine && !view.contains(&id) && view.len() < n {
                view.push(id);
            }
        }
    }
...
impl App for AupeDecay4 {
    ...
    fn handle(&mut self, net: Net, from: PeerRef, msg: &Self::Msg) {
        if self.is_byzantine {
            // Avant attack_start : les byzantins sont totalement silencieux.
            // Après attack_start : ils floodent et empoisonnent les PullReply.
            ...
            match msg {
                Msg::SelfNotif => {
                    ...
                    // avant attack_start : aucun push, les byzantins restent invisibles
                },
                Msg::PullRequest => {
                    if attacking {
                        net.send(from, Msg::PullReply(sample(&mut byzantines[..], self.params.view_size, &mut self.rng)));
                    }
                    // avant attack_start : pas de réponse (les byzantins ne sont pas dans les vues
                    // de toute façon, donc ce cas ne devrait pas se produire)
                },
                _ => (),
            }
        } else {
