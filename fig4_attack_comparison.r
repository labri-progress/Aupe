...
#
#   Camouflage (decay2) : les byzantins participent (se camouflent) avant le
#                         round 10 000, puis lancent l'attaque.
#   Soudaine   (decay4) : les byzantins sont absents avant le round 10 000,
...
#   Type de ligne → régime d'attaque (Camouflage = solide, Soudaine = tiretée)
#
...
attack_keys <- c("Camouflage" = "decay2", "Soudaine" = "decay4")
...
byz_plot <- function(data, f, show_legend = FALSE, show_y_title = TRUE) {
  ...
  p <- ggplot(data, aes(x      = time,
    ...
    mytheme +
    theme(legend.position = if (show_legend) c(0.68, 0.65) else "none") +
    guides(
      color    = guide_legend(title = "Merge (t)",  ncol = 1,
                              override.aes = list(linetype = "solid",
                                                  linewidth = 0.8)),
      linetype = guide_legend(title = "Attaque",    ncol = 1,
